...
stdenv.mkDerivation (finalAttrs: {
  ...
  # postPatch = ''
  #   substituteInPlace cmake_modules/ThirdpartyToolchain.cmake \
  #     --replace-fail \
  #       "set(ARROW_BOOST_COMPONENTS filesystem system)" \
  #       "set(ARROW_BOOST_COMPONENTS filesystem)"

  #   # Remove unconditional AWS SDK include
  #   sed -i '/include(AWSSDKVariables)/d' cmake_modules/ThirdpartyToolchain.cmake

  #   # Remove san-config include if missing
  #   sed -i '/include(san-config)/d' CMakeLists.txt

  #   # Inject a Boost::system stub before Boost is searched
  #   sed -i '1i \
  #   add_library(boost_system_interface INTERFACE)\
  #   add_library(Boost::system ALIAS boost_system_interface)' cmake_modules/ThirdpartyToolchain.cmake
