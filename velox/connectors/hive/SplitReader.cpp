...
std::vector<uint64_t> parseDeltaDeletionVectorRowIndexes(
  ...
  VELOX_USER_CHECK(
      splitInfo.isObject(),
      "Delta split extraFileInfo must be a JSON object.");
 ...
 public:
  ...
  uint64_t next(uint64_t size, VectorPtr& output) override {
    ...
    mutation.deletedRows =
        deleteBitmap_ && deleteBitmap_->size() > 0 ? deleteBitmap_->as<uint64_t>()
                                                   : nullptr;
...
std::unique_ptr<SplitReader> SplitReader::create(
  ...
  if (hiveSplit->customSplitInfo.count("table_format") > 0 &&
    ...
    return std::make_unique<DeltaSplitReader>(
      ...
      hiveSplit->customSplitInfo.count("table_format") > 0 &&
